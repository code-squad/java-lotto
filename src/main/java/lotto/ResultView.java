...
public class ResultView {
	...
	//거스름돈과 로또 구입금액을 알려준다.
	static void giveChangeMsg(int money, int change) {
	...
	static void printTicket(ArrayList<Lotto> tickets) {
		for (int i = 0; i < tickets.size(); i++) {
			System.out.println(tickets.get(i).getNum());
		}
	}
	//맞은 번호의 갯수만큼 결과를 출력해주는 메소드.
	static void printResult(ArrayList<Integer> result, int yield) {
