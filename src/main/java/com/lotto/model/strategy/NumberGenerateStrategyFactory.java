...
public class NumberGenerateStrategyFactory {
	...
	public enum GenerateTypes {
		...
		public static GenerateTypes values(String msg) {
			return Arrays.asList(GenerateTypes.values()).stream().filter(v -> v.supports(msg))
					.findFirst().orElseThrow(() -> new InvalidTypeOperationException("지원하지 않는 타입 호출입니다."));
		}
	}
	...
	public void initialize() {
		...
		reflections.getTypesAnnotatedWith(GenerateStrategy.class).stream()
		.forEach(c -> {
			try {
	...
	public NumberGenerateStrategy getStrategy(String input) {
		return this.ngsImpls.stream().filter(n -> n.supports(GenerateTypes.values(input)))
				.findFirst().orElseThrow(() -> new InvalidTypeOperationException("지원하지 않는 타입입니다."));
	}
}
