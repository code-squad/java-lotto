...
public class LottoGameTest {
    ...
    public void winningLotto_보너스_범위_밖() {
        WinningLotto.ofWinningLotto(convertLottoNums(Arrays.asList(1, 2, 3, 4, 5, 6)), 47);
    }

    @Test(expected = IllegalArgumentException.class)
    public void winningLotto_보너스_로또_중복수() {
        WinningLotto.ofWinningLotto(convertLottoNums(Arrays.asList(1, 2, 3, 4, 5, 6)), 6);
    }

    @Test(expected = IllegalArgumentException.class)
    public void WinningLotto_만들때_super받는_로또값_유효성체크_중복() {
        WinningLotto.ofWinningLotto(convertLottoNums(Arrays.asList(1, 2, 3, 5, 5, 45)), 6);
    }

    @Test(expected = IllegalArgumentException.class)
    public void WinningLotto_만들때_super받는_로또값_유효성체크_범위_초과() {
        WinningLotto.ofWinningLotto(convertLottoNums(Arrays.asList(1, 2, 3, 5, 5, 47)), 6);
    }

    @Test(expected = IllegalArgumentException.class)
    public void WinningLotto_만들때_super받는_로또값_유효성체크_수_초과() {
        WinningLotto.ofWinningLotto(convertLottoNums(Arrays.asList(1, 2, 3, 5, 5, 45, 13)), 6);
    }
